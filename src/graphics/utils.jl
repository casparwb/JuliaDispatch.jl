module Utils
    ...
    export _kw_extract, get_n_snapshots, get_unit, get_available_ivs, get_snapshot_time, get_snapshot_ids
    ...
    Returns the total number of snapshots in the given `data/run` folder.
    """
    ...
    function get_snapshot_ids(;data="../data", run="")
        ...
        folders = [folder for folder in readdir(datadir) if (isdir(folder) && startswith(folder, "0"))]
